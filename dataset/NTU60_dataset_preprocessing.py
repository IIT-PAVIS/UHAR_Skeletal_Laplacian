...
class NTU60_preprocess:
    def __init__(self):
        ...
        self.skeletons_ignored_file = '{}/raw/ntu_60/samples_with_missing_skeletons.txt'.format(self.save_path)
        ...
        # Step 7/7: Train-Test Split by Cross-Subject and Cross-View
        self.data_split()
    ...
    def get_raw_skeletons_data(self):
        ...
        self.camera = []  # Camera ID 1~3
        self.performer = []  # Subject ID 1~40
        for filename in self.skeletons_name:
            self.camera.append(int(filename[filename.find('C') + 1:filename.find('C') + 4]))
            self.performer.append(int(filename[filename.find('P') + 1:filename.find('P') + 4]))
        self.camera = np.array(self.camera, dtype=np.int)
        ...
        with open('{}/camera.pkl'.format(self.save_path), 'wb') as fc:
            pickle.dump(self.camera, fc)
    ...
    def data_split(self):
        ...
        # Cross View Split
        xview_split_path = '{}/xview'.format(self.save_path)
        if not osp.exists(xview_split_path):
            os.mkdir(xview_split_path)

        self.xview_train_data, self.xview_test_data = [], []
        self.xview_train_label, self.xview_test_label = [], []
        for i in tqdm(range(len(self.final_data)),
                      desc='Step 7/7: Cross-View Train-Test Split'):

            # Test set
            if self.camera[i] == 1:
                self.xview_test_data.append(torch.tensor(self.final_data[i].reshape(-1, 25*self.actors, 3).transpose(2, 1, 0)).float())
                self.xview_test_label.append(torch.tensor(self.label[i] - 1))

            # Train set
            else:
                self.xview_train_data.append(torch.tensor(self.final_data[i].reshape(-1, 25*self.actors, 3).transpose(2, 1, 0)).float())
                self.xview_train_label.append(torch.tensor(self.label[i] - 1))

        torch.save(self.xview_train_data,
                   '{}/train_data_{}.pt'.format(xview_split_path, self.actors))
        torch.save(self.xview_train_label,
                   '{}/train_label_{}.pt'.format(xview_split_path, self.actors))
        torch.save(self.xview_test_data,
                   '{}/test_data_{}.pt'.format(xview_split_path, self.actors))
        torch.save(self.xview_test_label,
                   '{}/test_label_{}.pt'.format(xview_split_path, self.actors))
